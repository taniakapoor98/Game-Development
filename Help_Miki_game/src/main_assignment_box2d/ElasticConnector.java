...
public class ElasticConnector {
	...
	public ElasticConnector(BasicParticle p1, BasicParticle p2, double naturalLength, double springConstant, double vibrationalDampingConstant, 
		...
		MainEngine.world.createJoint(jd);// add the distanceJoint to the world

	}
	

	public void draw(Graphics2D g) {
		int x1 = MainEngine.convertWorldXtoScreenX(particle1.body.getPosition().x);
		int y1 = MainEngine.convertWorldYtoScreenY(particle1.body.getPosition().y);
		int x2 = MainEngine.convertWorldXtoScreenX(particle2.body.getPosition().x);
		int y2 = MainEngine.convertWorldYtoScreenY(particle2.body.getPosition().y);
		g.setColor(col);
